...
- generator retrained: yes -- scroll, swipe, from the released run's own `effective_config.json` with the switch above flipped (`a8_no_feature/plan.json`, `a8_no_feature/config_<action>.yaml`)
...
|---|---|---|---|---|
| `train_scroll.log` | **0** | 606 | 606 | 606 on each of 6 keys |
| `train_swipe.log` | **0** | 472 | 472 | 472 on each of 6 keys |
...
The plan its driver wrote for these runs asked for exactly that: `critics.feature` `true` -> `false` (scroll, swipe).

The gradient protection is A7's variable, not this arm's, and the plan records it unchanged at the release's own `g_weight: 0.04`, `max_grad_ratio: 0.5`, `project_conflicts: true` for every run here.

The zero column is the removal: `adv_acc_feature` never appears, while `adv_acc_set`, `adv_acc_waveform` appear on every logged step.

`adv_feature_match_*` is still counted on every step, which is this arm's honest limit: it removes a critic, not the direct feature-matching loss that runs in the same `adv_update` block. Because the critic removed here is the feature one, the feature pathway is only half removed and the direct match keeps supplying the same statistics -- that other half is A11's variable.
