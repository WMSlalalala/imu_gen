# A11: the direct feature-matching loss
...
- generator retrained: yes -- scroll, swipe, from the released run's own `effective_config.json` with the switch above flipped (`a11_no_feature_match/plan.json`, `a11_no_feature_match/config_<action>.yaml`)
...
|---|---|---|---|---|
| `train_scroll.log` | 605 | 605 | 605 | **0** |
| `train_swipe.log` | 472 | 472 | 472 | **0** |

The plan its driver wrote for these runs asked for exactly that: `feature_match_weight` `0.03` -> `0.0` (scroll); `feature_match_weight` `0.02` -> `0.0` (swipe).
...
`adv_feature_match_*` is the removal here -- the whole group is absent -- while `adv_acc_feature`, `adv_acc_set`, `adv_acc_waveform` keep their counts, so this arm moved the loss weight and left the critics running.
