...
- generator retrained: yes -- scroll, swipe, from the released run's own `effective_config.json` with the switch above flipped (`a10_no_waveform/plan.json`, `a10_no_waveform/config_<action>.yaml`)
...
|---|---|---|---|---|
| `train_scroll.log` | 605 | 605 | **0** | 605 on each of 6 keys |
| `train_swipe.log` | 472 | 472 | **0** | 472 on each of 6 keys |
...
The plan its driver wrote for these runs asked for exactly that: `critics.waveform` `true` -> `false` (scroll, swipe).
...
The zero column is the removal: `adv_acc_waveform` never appears, while `adv_acc_feature`, `adv_acc_set` appear on every logged step.

`adv_feature_match_*` is still counted on every step, which is this arm's honest limit: it removes a critic, not the direct feature-matching loss that runs in the same `adv_update` block.
