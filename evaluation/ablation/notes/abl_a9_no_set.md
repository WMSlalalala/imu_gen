...
- generator retrained: yes -- scroll, swipe, from the released run's own `effective_config.json` with the switch above flipped (`a9_no_set/plan.json`, `a9_no_set/config_<action>.yaml`)
...
|---|---|---|---|---|
| `train_scroll.log` | 605 | **0** | 605 | 605 on each of 6 keys |
| `train_swipe.log` | 472 | **0** | 472 | 472 on each of 6 keys |
...
The plan its driver wrote for these runs asked for exactly that: `critics.set` `true` -> `false` (scroll, swipe).
...
The zero column is the removal: `adv_acc_set` never appears, while `adv_acc_feature`, `adv_acc_waveform` appear on every logged step.
